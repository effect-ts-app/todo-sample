...
import { useFindTaskList } from "../data"
...
function TaskListEntry(l: { id: string; title: string }) {
  const cat = "a"
  const [r] = useFindTaskList(l.id)
  const count = datumEither.isSuccess(r) ? r.value.right.items.length : null
  return (
    <Link href={`/${l.id}`} passHref>
      <ListItem sx={{ pl: 4 }} button selected={l.id === cat}>
        {l.title} {count ? `(${count})` : null}
      </ListItem>
    </Link>
  )
}
