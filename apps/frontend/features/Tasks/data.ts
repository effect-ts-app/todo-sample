...
const newTask = (v: TaskView, folderId: O.Option<UUID>) => (newTitle: string) =>
  TodoClient.Tasks.createTaskE({
    title: newTitle,
    isFavorite: false,
    myDay: null,
    folderId: O.toNullable(folderId),
    ...(v === "important"
      ? { isFavorite: true }
      : v === "my-day"
...
export function useNewTask(v: TaskView, folderId: O.Option<UUID>) {
